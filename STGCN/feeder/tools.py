...
def auto_pading(data_numpy, size, random_pad=False): #对数据进行填充（padding）操作。函数的目的是确保数据的时间步数（T）符合指定的大小（size）。如果数据的时间步数小于目标大小，它会填充数据到目标大小；否则，数据保持不变。
    # 传入的 size 就是 window_size = 300
    C, T, V = data_numpy.shape #[3,300,18,2]


    if T < size: 
        print(' def auto_pading  if T < size:')
...
# def random_move(data_numpy, #有点复杂
...
def random_move(data_numpy,
    ...
    # Perform transformation
    for i_frame in range(T):
        xyz = data_numpy[:, i_frame, :]  # Extract x, y, z
        new_xyz = np.dot(theta[:, :, i_frame], xyz.reshape(2, -1))
        new_xyz[0] += t_x[i_frame]
        new_xyz[1] += t_y[i_frame]
        data_numpy[:, i_frame, :] = new_xyz.reshape(3, V)  # Update data
...
def random_shift(data_numpy):
    ...
    # Check for valid frames (frames with non-zero data)
